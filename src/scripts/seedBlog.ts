 ...
 *   npx tsx src/scripts/seedBlog.ts
 *
 * Seeds the first blog post into Firestore.
 */
...
async function seed() {
  ...
  if (!existing.empty) {
    console.log('⚠️  Blog post already exists — skipping seed.');
    process.exit(0);
  }
  ...
  console.log(`✅ Blog post seeded successfully! Document ID: ${docRef.id}`);
  process.exit(0);
}
